...
\begin{enumerate}
   ...
   \begin{enumerate}
      ...
      \item (Baris 6--7) Mendapatkan \textit{host} dari URL awal, lalu masukkan URL awal ini ke dalam antrean \texttt{frontier} sebagai titik awal pemeriksaan.
      
      \item (Baris 9) Menjalankan \textit{loop} untuk memproses isi dari antrean \texttt{frontier}. \textit{Loop} akan terus berlangsung selama \texttt{frontier} belum kosong, tidak dihentikan oleh pengguna dan jumlah tautan belum melebihi batas.
      
      \item (Baris 10--11) Mengambil elemen pertama dari antrean menggunakan \texttt{poll()}. Jika nilai yang diambil adalah \texttt{null}, proses langsung dilanjutkan ke iterasi berikutnya.
      
      \item (Baris 13--14) Memasukkan tautan saat ini ke \texttt{repositories}, jika mengembalikan \texttt{null} artinya tautan ini sudah ada dalam daftar dan tidak perlu diperiksa lagi.
      
      \item (Baris 16) Melakukan pemeriksaan terhadap tautan saat ini dan mengirim perintah agar \textit{response body} dilakukan \textit{parsing} untuk mendapatkan dokumen HTML.
      
      \item (Baris 18) Jika tautan bukan merupakan tautan halaman situs web, maka lanjutkan ke iterasi berikutnya.
      
      \item (Baris 20) Melakukan ekstraksi seluruh tautan yang terdapat pada tautan halaman.
      
      \item (Baris 21) Membuat \textit{executor service} berbasis \textit{virtual thread} sehingga setiap pemeriksaan tautan dapat dilakukan dalam \textit{thread} terpisah.
      
      \item (Baris 22) Melakukan \textit{looping} terhadap setiap tautan hasil ekstraksi.
      
      \item (Baris 23--26) Jika jumlah tautan sudah melebihi batas maka hentikan \textit{looping} dan kosongkan \textit{frontier}.
      
      \item (Baris 27--28) Mengambil objek \texttt{Link} hasil ekstraksi beserta teks \textit{anchor}-nya.
      
      \item (Baris 29--34) Jika tautan sudah dikunjungi, maka tambahkan relasi antara tautan yang lama dengan halaman saat ini, jika belum maka tambahkan relasi tautan saat ini dengan halaman saat ini.
      
      \item (Baris 36--37) Jika tautan memiliki \textit{host} yang sama dengan \texttt{rootHost} maka ditambahkan ke dalam \texttt{frontier} karena tautan tersebut berpotensi menjadi tautan halaman.
      
      \item (Baris 39) Masukan tautan ke dalam \texttt{repositories} jika belum.
   ...
   \begin{enumerate}
      \item (Baris 3--4) Membuat atau mengambil objek \texttt{RateLimiter} berdasarkan \textit{host} dari URL, lalu menerapkan \textit{rate limiting}.
      
      \item (Baris 5) Melakukan \textit{fetching} pada tautan.
      
      \item (Baris 6--8) Memperbarui nilai dari atribut objek \texttt{Link}.
      
      \item (Baris 10) Jika diminta untuk melakukan \textit{parsing}, \textit{fetching} berhasil, dan berpotensi sebagai tautan halaman.
      
      \item (Baris 11--20) Dapatkan \textit{response body} dalam format \texttt{String}, lalu \textit{parse} ke HTML, jika berhasil maka tandai tautan saat ini sebagai halaman, lalu kembalikan dokumen HTML.
      
      \item (Baris 21--23) Jika terjadi \textit{error} maka perbarui atribut \texttt{error} pada objek tautan berdasarkan jenis \textit{error} yang dilempar.
      
      \item (Baris 25) Kirim tautan ke \texttt{MainController} apapun hasil pemeriksaannya.
   ...
   \begin{enumerate}[itemsep=3pt]
      ...
      \item (Baris 13) Mengambil teks yang berada di dalam \textit{tag} \texttt{a} dan menghapus spasi berlebih pada bagian ujung teks.
