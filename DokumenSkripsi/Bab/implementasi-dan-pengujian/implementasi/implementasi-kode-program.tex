...
\begin{enumerate}
   ...
   \begin{enumerate}
      ...
      \item \texttt{linkSender}: Atribut ini menyimpan sebuah fungsi \textit{callback} untuk mengirim objek \texttt{Link} kembali ke kelas \texttt{MainController}. Implementasi dari fungsi ini menggunakan \textit{functional interface} \texttt{Consumer} sehingga cukup mengirim satu buah parameter tanpa memerlukan pengembalian nilai (\textit{return}).
   ...
   \begin{enumerate}[itemsep=3pt]
      ...
      \item (Baris 7--8) Mendapatkan \textit{host} dari URL awal, lalu masukkan URL awal ini ke dalam antrean \texttt{frontier} sebagai titik awal pemeriksaan.
      
      \item (Baris 10) Menjalankan \textit{loop} untuk memproses isi dari antrean \texttt{frontier}. \textit{Loop} akan terus berlangsung selama \texttt{frontier} belum kosong, tidak dihentikan oleh pengguna dan jumlah tautan belum melebihi batas.
      
      \item (Baris 11--12) Mengambil elemen pertama dari antrean menggunakan \texttt{poll()}. Jika nilai yang diambil adalah \texttt{null}, proses langsung dilanjutkan ke iterasi berikutnya.
      
      \item (Baris 14) Melakukan pemeriksaan terhadap tautan saat ini dan mengirim perintah agar \textit{response body} dilakukan \textit{parsing} untuk mendapatkan dokumen HTML.
      
      \item (Baris 16) Jika tautan bukan merupakan tautan halaman situs web atau dokumen HTML kosong, maka lanjutkan ke iterasi berikutnya.
      
      \item (Baris 18) Melakukan ekstraksi seluruh tautan yang terdapat pada tautan halaman.
      
      \item (Baris 20) Membuat \textit{executor service} berbasis \textit{virtual thread} sehingga setiap pemeriksaan tautan dapat dilakukan dalam \textit{thread} terpisah.
      
      \item (Baris 21) Melakukan \textit{looping} terhadap setiap tautan hasil ekstraksi.
      
      \item (Baris 22--25) Jika jumlah tautan sudah melebihi batas atau proses dihentikan pengguna, maka hentikan \textit{looping} dan kosongkan \textit{frontier}.
      
      \item (Baris 27--28) Mengambil objek \texttt{Link} hasil ekstraksi beserta teks \textit{anchor}-nya.
      
      \item (Baris 30--34) Jika tautan sudah dikunjungi, maka tambahkan relasi antara tautan yang lama dengan tautan halaman, lalu lanjutkan ke iterasi berikutnya.
      
      \item (Baris 35) Tambahkan relasi antara tautan saat ini dengan tautan halaman.
      
      \item (Baris 37--38) Jika tautan memiliki \textit{host} yang sama dengan \texttt{rootHost} maka ditambahkan ke dalam \texttt{frontier} karena tautan tersebut berpotensi menjadi tautan halaman.
      
      \item (Baris 40) Mengirim tugas ke \textit{executor service} agar dibuatkan \textit{virtual thread} untuk menjalankan tugas ini.
      
      \item (Baris 41) Melakukan pemeriksaan terhadap tautan saat ini dan mengirim perintah agar \textit{response body} tidak dilakukan \textit{parsing}.
      
      \item (Baris 45) Menutup \textit{executor service} agar tidak bisa menerima tugas baru.
      
      \item (Baris 46) Tunggu sampai semua tugas yang sudah dibuat selesai, baru bisa melanjutkan ke antrean \texttt{frontier} berikutnya.
      
      \item (Baris 47--49) Jika terjadi \textit{error} maka tandai setiap \textit{thread} aktif bahwa \textit{thread} tersebut sudah dihentikan.
   \end{enumerate}
   ...
   \begin{enumerate}[itemsep=4pt]
      ...
      \item (Baris 3--4) Jika tautan sudah pernah diperiksa atau jumlah tautan keseluruhan sudah melebihi batas, maka keluar dari metode.
      
      \item (Baris 8) Mengambil atau membuat objek \texttt{RateLimiter} berdasarkan \textit{host} dari URL tautan. Sehingga penerapan \textit{rate limiting} dapat dilakukan per \textit{host}.
      
      \item (Baris 9) Menerapkan \textit{rate limiting} per \textit{host}.
      
      \item (Baris 11) Melakukan permintaan HTTP pada tautan.
      
      \item (Baris 13--15) Memperbarui nilai dari atribut objek \texttt{Link}, seperti \textit{final} URL, tipe konten, kode status HTTP.
      
      \item (Baris 18) Menyiapkan variabel untuk menyimpan dokumen HTML hasil \textit{parsing}.
      
      \item (Baris 20) Menetapkan syarat \textit{parsing}. Jika permintaan HTTP berhasil dan \textit{response body} tidak kosong.
      
      \item (Baris 21) Menetapkan syarat \textit{parsing}. Jika \textit{final} URL memiliki \textit{host} yang sama dengan \textit{host} dari URL awal.
      
      \item (Baris 23) Jika diminta untuk \textit{parsing} dan semua syarat \textit{parsing} terpenuhi.
      
      \item (Baris 24) Mengambil \textit{response body} dalam bentuk \texttt{String} agar bisa di-\textit{parsing}.
      
      \item (Baris 26) Memulai blok \texttt{try} internal untuk menangani kemungkinan kegagalan selama proses \textit{parsing}.
      
      \item (Baris 27) Melakukan \textit{parsing} \textit{response body} menjadi objek \texttt{Document} HTML menggunakan pustaka \texttt{Jsoup}.
      ...
      \item (Baris 30) Jika terjadi \texttt{exception} atau \texttt{error}, nama tipe kesalahan disimpan ke atribut \texttt{error} pada objek \texttt{Link} untuk tujuan pelaporan.
      ...
      \item (Baris 40--43) Jika tautan baru berhasil ditambahkan ke \texttt{repositories}, maka kirim tautan ke \texttt{MainController} untuk ditampilkan ke GUI.
   ...
   \begin{enumerate}[itemsep=3pt]
      ...
      \item (Baris 2) Membuat objek \texttt{Map} untuk menyimpan hasil ekstraksi tautan, dengan \texttt{key} berupa objek \texttt{Link} dan \texttt{value} berupa teks yang berada pada tautan tersebut.
   ...
   \begin{enumerate}[itemsep=4pt]
      ...
      \item (Baris 15--18) Jika normalisasi diminta untuk dilakukan secara ketat, maka \texttt{scheme} dan \texttt{host} tidak boleh \texttt{null} atau \texttt{String} kosong.
      ...
      \item (Baris 30) Membentuk kembali objek \texttt{URI} baru. Komponen \texttt{userInfo} dan \texttt{fragment} tidak disertakan karena keduanya tidak diperlukan dalam proses pemeriksaan tautan.

      \item (Baris 32) Mengembalikan URL hasil normalisasi dalam format ASCII untuk menghindari karakter non-ASCII yang tidak kompatibel dengan \texttt{HttpClient}.

      \item (Baris 34--36) Jika terjadi \textit{error} maka kembalikan URL awal sehingga dapat teridentifikasi sebagai tautan rusak pada tahap \textit{fetching}. Tindakan ini dilakukan untuk menghindari \textit{false negative}, karena jika dikembalikan \texttt{null} maka tautan ini tidak akan diperiksa.
   ...
   \item \textbf{Berkas \texttt{HttpHandler.java}}\\
   ...
   \begin{enumerate}
      ...
      \item (Baris 3) Melakukan permintaan HTTP tanpa menyimpan \textit{response body} agar pemeriksaan lebih cepat jika kontennya bukan HTML. 
      ...
      \item (Baris 8--10) Jika \textit{response body} dibutuhkan dan kontennya adalah HTML maka dilakukan pemeriksaan ulang namun kali ini \textit{response body} diambil.
   ...
   Kelas \texttt{Link} merupakan model yang digunakan untuk merepresentasikan tautan yang ditemukan selama proses pemeriksaan. Atribut dari kelas ini terdiri atas \texttt{url} untuk menyimpan URL awal, \texttt{finalUrl} untuk menyimpan URL hasil \textit{redirect}, \texttt{statusCode} untuk menyimpan kode status HTTP, \texttt{contentType} untuk menyimpan jenis konten yang dikembalikan URL, \texttt{error} untuk menyimpan pesan \textit{error}, \texttt{isWebpage} untuk menentukan apakah URL merupakan halaman situs web atau bukan, serta \texttt{relations} untuk menyimpan daftar tautan lain yang berelasi dengan objek tautan ini. Seluruh atribut pada kelas ini kecuali \texttt{relations}, didefinisikan menggunakan JavaFX \textit{properties} agar nilai pada atribut dapat diperbarui secara otomatis di antarmuka. Kode lengkap untuk kelas ini terdapat pada Lampiran~\ref{code:link-java}, namun pada bagian ini yang akan dijelaskan lebih rinci adalah metode \texttt{addRelation}.

   Metode \texttt{addRelation} merupakan metode yang digunakan untuk menambahkan objek tautan lain sebagai relasi dari objek tautan saat ini. Masukan dari metode ini adalah sebuah objek tautan yang ingin ditambahkan ke dalam daftar relasi. Berikut adalah penjelasan lebih rinci untuk metode \texttt{addRelation} (lihat Kode~\ref{code:link-add-relation-java}):
   ...
   \begin{enumerate}
      ...
      \item (Baris 6) Menambahkan objek \texttt{other} sebagai relasi dari objek saat ini. Untuk mencegah terjadinya duplikasi dalam pembuatan relasi maka digunakan metode \texttt{putIfAbsent}. Jika \texttt{anchorText} bernilai \texttt{null}, teks yang disimpan diganti menjadi \texttt{String} kosong.
   ...
   \lstinputlisting[
      language=Java, 
      caption=Kelas \texttt{Link} metode \texttt{addRelation},
      label={code:link-add-relation-java}
   ]{Lampiran/implementasi-kode-program/Link_addRelation.java}
