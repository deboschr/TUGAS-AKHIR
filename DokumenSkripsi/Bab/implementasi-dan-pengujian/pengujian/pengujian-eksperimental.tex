...
Berdasarkan rangkaian pengujian yang telah dilakukan terhadap tiga parameter operasional, yaitu \texttt{INTERVAL}, \texttt{CONNECTION\_TIMEOUT}, dan \texttt{REQUEST\_TIMEOUT}, diperoleh kesimpulan bahwa setiap parameter memiliki pengaruh yang berbeda terhadap durasi pemeriksaan dan hasil identifikasi tautan rusak. Pada pengujian \texttt{INTERVAL}, seluruh variasi nilai menghasilkan jumlah tautan rusak yang relatif identik sehingga parameter ini tidak berpengaruh terhadap akurasi pemeriksaan, dan nilai terbaik ditetapkan pada \textbf{0~milidetik} karena memberikan durasi tercepat. Pada pengujian \texttt{CONNECTION\_TIMEOUT}, nilai yang terlalu kecil menghasilkan banyak \textit{error} \textit{Timeout}, sedangkan nilai yang lebih besar memberikan hasil yang lebih stabil, sehingga nilai terbaik adalah \textbf{20 detik}. Pola yang sama juga ditemukan pada pengujian \texttt{REQUEST\_TIMEOUT}, di mana nilai yang rendah menyebabkan kegagalan permintaan HTTP dan meningkatnya jumlah \textit{error} \textit{Timeout}, sementara hasil mulai stabil pada nilai 20 detik. Dengan demikian, nilai terbaik untuk parameter ini juga adalah \textbf{20 detik}.

Selain pengaruh dari parameter operasional yang diuji, hasil pengujian juga menunjukkan bahwa kondisi jaringan internet memiliki peran yang signifikan terhadap durasi dan konsistensi hasil pemeriksaan. Pada beberapa percobaan ditemukan anomali baik pada durasi pemeriksaan maupun pada pengelompokan kategori \textit{error}, yang tidak sepenuhnya dapat dijelaskan oleh variasi nilai parameter yang diuji. Anomali tersebut diduga kuat dipengaruhi oleh faktor eksternal, khususnya kecepatan dan kestabilan koneksi jaringan internet pada saat pengujian berlangsung. Oleh karena itu, disarankan agar aplikasi Broken Link Scanner dijalankan pada kondisi jaringan internet yang cepat dan stabil untuk meminimalkan terjadinya \textit{Timeout}, menjaga konsistensi hasil pemeriksaan, serta memperoleh durasi pemeriksaan yang lebih cepat.
