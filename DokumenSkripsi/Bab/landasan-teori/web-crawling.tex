...
\begin{enumerate}[itemsep=5pt]
  ...
  \item \textbf{Mengambil URL dari \textit{frontier}}: Sebuah URL diambil dari \textit{frontier} mengikuti strategi antrean yang digunakan, seperti FIFO untuk \textit{breadth-first crawler} atau \textit{priority queue} untuk \textit{preferential crawler}.
...
\begin{itemize}[itemsep=-1pt]
  ...
  \item \textbf{Pemblokiran oleh \textit{Server}} \\
  \textit{Server} dapat memblokir alamat IP yang dianggap terlalu agresif atau tidak mematuhi aturan akses. Meskipun \texttt{robots.txt} tidak bersifat memaksa, \textit{server} tetap dapat menolak permintaan dari \textit{crawler} yang tidak mengikuti kebijakan yang ditetapkan.
