...
\begin{enumerate}
    ...
    Elemen semantik adalah elemen yang memiliki makna jelas bagi \textit{browser} maupun pembaca, karena nama elemen menggambarkan fungsinya. Contoh elemen semantik adalah sebagai berikut:
    \begin{itemize}[itemsep=1mm]
        \item \texttt{<header>}: Elemen untuk bagian kepala suatu halaman atau bagian.
        \item \texttt{<nav>}: Elemen untuk kumpulan tautan navigasi.
        \item \texttt{<section>}: Elemen untuk sebuah bagian tematik dalam dokumen.
        \item \texttt{<article>}: Elemen untuk konten yang berdiri sendiri, seperti artikel berita.
        \item \texttt{<aside>}: Elemen untuk konten samping, seperti catatan atau iklan.
        \item \texttt{<footer>}: Elemen untuk bagian kaki halaman.
    \end{itemize}

    \item \textbf{Elemen Non-Semantik} \\
    Elemen non-semantik adalah elemen yang tidak menggambarkan makna spesifik dari kontennya, melainkan digunakan untuk keperluan pemformatan atau pengelompokan. Contoh elemen non-semantik adalah sebagai berikut:
    \begin{itemize}[itemsep=1mm]
        \item \texttt{<div>}: Elemen untuk pengelompokan blok konten.
        \item \texttt{<span>}: Elemen untuk pengelompokan teks dalam baris.
    \end{itemize}
\end{enumerate}  

\subsection{Atribut Global dan Spesifik}
\label{subsec:0224-atribut-global-dan-spesifik}
Setiap elemen HTML dapat memiliki atribut yang berfungsi untuk memberikan informasi tambahan atau mengatur perilaku elemen tersebut. Atribut terbagi menjadi dua kategori, yaitu atribut global dan atribut spesifik. Atribut global dapat digunakan pada hampir semua elemen HTML. Atribut ini bersifat umum karena tidak terikat pada fungsi tertentu dari elemen. Sedangkan, atribut spesifik adalah atribut yang hanya berlaku pada elemen tertentu sesuai dengan fungsinya.

\vspace{2mm}

Berikut adalah beberapa contoh dari atribut global:
\begin{itemize}[itemsep=1mm]
...
Tabel~\ref{tab:html-specific-attributes} menunjukkan beberapa contoh atribut spesifik beserta elemen tempat atribut tersebut digunakan.

\renewcommand{\arraystretch}{1.5}
\begin{table}[H]
\centering
\caption{Atribut spesifik HTML yang memuat URL}
\label{tab:html-specific-attributes}
\begin{tabular}{|l|l|p{7cm}|}
\hline
\textbf{Elemen} & \textbf{Atribut} & \textbf{Keterangan} \\ \hline
\texttt{<a>} & \texttt{href} & Menentukan alamat tujuan tautan menuju dokumen atau sumber daya lain. \\ \hline
\texttt{<img>} & \texttt{src} & Menentukan lokasi berkas gambar yang akan ditampilkan. \\ \hline
\texttt{<link>} & \texttt{href} & Menentukan alamat sumber daya eksternal, seperti stylesheet. \\ \hline
\texttt{<script>} & \texttt{src} & Menentukan alamat berkas JavaScript eksternal. \\ \hline
\texttt{<form>} & \texttt{action} & Menentukan alamat tujuan pengiriman data formulir. \\ \hline
\texttt{<iframe>} & \texttt{src} & Menentukan alamat dokumen yang dimuat di dalam bingkai. \\ \hline
\texttt{<video>} & \texttt{src} & Menentukan lokasi berkas video. \\ \hline
\texttt{<audio>} & \texttt{src} & Menentukan lokasi berkas audio. \\ \hline
\texttt{<source>} & \texttt{src} & Menentukan lokasi sumber media alternatif. \\ \hline
\texttt{<track>} & \texttt{src} & Menentukan lokasi berkas teks trek, seperti subtitle. \\ \hline
\end{tabular}
\end{table}
