...
\begin{itemize}[itemsep=1mm]
    \item \texttt{uri}\\
    Metode ini digunakan untuk menentukan alamat tujuan dari \textit{request} HTTP dalam bentuk objek \texttt{URI}. URI ini menjadi acuan utama bagi \texttt{HttpClient} dalam menentukan lokasi sumber daya yang akan diakses.

    \item \texttt{header}\\
    Metode ini digunakan untuk menetapkan satu pasangan \textit{name-value} sebagai \textit{header} pada \textit{request}. Apabila diperlukan penetapan beberapa \textit{header} sekaligus, dapat digunakan metode \texttt{headers} agar konfigurasi \textit{request} menjadi lebih ringkas.

    \item \texttt{timeout}\\
    Metode ini menerima parameter berupa objek \texttt{Duration} untuk menentukan batas waktu maksimal pemrosesan \textit{request}. Jika \textit{response} tidak diterima dalam durasi yang ditentukan, permintaan akan dihentikan dan dianggap gagal.

    \item \texttt{version}\\
    Metode ini digunakan untuk menentukan versi protokol HTTP yang akan digunakan pada proses \textit{request}. Pengaturan versi ini dapat menimpa konfigurasi versi default yang ditetapkan pada objek \texttt{HttpClient}.

    \item \texttt{method}\\
    Metode ini digunakan untuk menetapkan jenis operasi HTTP yang akan dilakukan pada \textit{request}. Selain menggunakan metode ini secara langsung, penetapan operasi HTTP juga dapat dilakukan melalui metode yang lebih eksplisit, seperti \texttt{GET}, \texttt{POST}, \texttt{PUT}, dan \texttt{DELETE}, yang secara semantik merepresentasikan jenis permintaan yang dikirim.
\end{itemize}
...
\begin{itemize}[itemsep=1mm]
    ...
    Metode ini digunakan untuk memperoleh kode status HTTP yang dikirim oleh \textit{server}. Kode status ini menunjukkan hasil pemrosesan \textit{request}, seperti keberhasilan, pengalihan, atau kegagalan.

    \item \texttt{headers}\\
    Metode ini digunakan untuk mendapatkan seluruh \textit{header} yang disertakan dalam \textit{response}. Informasi ini dapat digunakan untuk mengetahui metadata tambahan yang dikirim oleh \textit{server}.

    \item \texttt{body}\\
    Metode ini digunakan untuk mendapatkan isi \textit{response body} dengan tipe data generik \texttt{<T>}. Tipe data ini bergantung pada \texttt{BodyHandler} yang digunakan saat \textit{request} dikirim.

    \item \texttt{previousResponse}\\
    Metode ini digunakan untuk memperoleh \textit{response} sebelumnya apabila terjadi \textit{redirect}. Dengan metode ini, riwayat \textit{redirect} dapat ditelusuri apabila diperlukan.

    \item \texttt{uri}\\
    Metode ini digunakan untuk mendapatkan URL tujuan akhir dari \textit{request}, termasuk apabila terjadi \textit{redirect} selama proses komunikasi.

    \item \texttt{version}\\
    Metode ini digunakan untuk mengetahui versi protokol HTTP yang digunakan dalam komunikasi antara \textit{client} dan \textit{server}.
\end{itemize}
