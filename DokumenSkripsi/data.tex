...
\\
Hasil pengujian menunjukkan bahwa aplikasi dapat melakukan pemeriksaan secara efektif dan konsisten. Pengujian lebih lanjut mengungkap bahwa pengaturan waktu tunggu ketika menghubungi \textit{server} dan waktu tunggu untuk mendapatkan respons dari \textit{server} pada permintaan HTTP berpengaruh secara langsung terhadap ketepatan hasil pemeriksaan. Waktu tunggu yang terlalu singkat menyebabkan lebih banyak tautan yang keliru teridentifikasi sebagai tidak dapat diakses, sedangkan waktu tunggu yang lebih panjang menghasilkan hasil yang lebih stabil meskipun mengorbankan waktu pemeriksaan yang menjadi lebih lama.}

\abstrakENG{Broken links on a website can disrupt information accessibility and diminish the overall user experience. To address this issue, this study develops a desktop-based application capable of detecting broken links on websites. The application performs \textit{web crawling} by traversing each page within a website and extracting the hyperlinks found on those pages. Each link is then examined through an HTTP request to determine whether it remains accessible, and the results are presented in real time through a graphical user interface.
\\
Experimental results show that the application can perform link checking effectively and consistently. Further testing reveals that the configuration of waiting time when establishing a connection to the server and waiting for the server’s response in HTTP requests has a direct impact on the accuracy of the inspection results. Short waiting times cause more links to be incorrectly identified as inaccessible, whereas longer waiting times produce more stable results, although with the trade-off of increased inspection duration.}
...
% Petunjuk: hilangkan tanda << & >>, dan isi sesuai dengan data anda.
%=============================================================================
\kunciINA{tautan rusak, tautan halaman, web crawling, pemeriksaan tautan, algoritma BFS, aplikasi desktop}
\kunciENG{broken links, webpage links, web crawling, link checking, BFS algorithm, desktop application}
