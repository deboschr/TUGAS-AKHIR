\renewcommand{\arraystretch}{1.3}
\begin{longtable}{
   |>{\raggedright\arraybackslash}m{4cm}
   |>{\raggedright\arraybackslash}m{6cm}
   |>{\centering\arraybackslash}m{2cm}|}
\caption{(Pengujian 1) Percobaan \texttt{INTERVAL} 2000~milidetik}
\vspace{-3mm}
\label{tab:percobaan-interval-2000} \\
\hline

{\centering\arraybackslash\textbf{Kategori}} &
{\centering\arraybackslash\textbf{Error}} &
{\centering\arraybackslash\textbf{Jumlah}} \\ \hline
\endfirsthead

\multicolumn{3}{c}{Tabel~\ref{tab:percobaan-interval-2000} dilanjutkan dari halaman sebelumnya}\\[4pt]

\hline
{\centering\arraybackslash\textbf{Kategori}} &
{\centering\arraybackslash\textbf{Error}} &
{\centering\arraybackslash\textbf{Jumlah}} \\ \hline
\endhead

\hline
\multicolumn{3}{|r|}{Bersambung ke halaman berikutnya} \\
\hline
\endfoot

\hline
\endlastfoot

% ********************************************

\multirow{6}{*}{\textbf{Connection Error}}
 & Connection Reset & 1 \\ \cline{2-3}
 & Host Not Found & 14 \\ \cline{2-3}
 & I/O Error & 1 \\ \cline{2-3}
 & Invalid URL & 1 \\ \cline{2-3}
 & SSL Error & 6 \\ \cline{2-3}
 & Timeout & 3 \\ \hline

\multicolumn{2}{|c|}{\centering\textbf{Total Connection Error}} &
{\centering\arraybackslash\textbf{26}} \\ \hline

% ********************************************

\multirow{5}{*}{\textbf{4XX Client Error}}
 & 400 Bad Request & 2 \\ \cline{2-3}
 & 403 Forbidden & 12 \\ \cline{2-3}
 & 404 Not Found & 29 \\ \cline{2-3}
 & 410 Gone & 1 \\ \cline{2-3}

\multicolumn{2}{|c|}{\centering\textbf{Total 4XX Client Error}} &
{\centering\arraybackslash\textbf{44}} \\ \hline

% ********************************************

\multirow{2}{*}{\textbf{Non-Standard Error}}
 & 520 & 2 \\ \cline{2-3}
 & 999 & 2 \\ \hline

\multicolumn{2}{|c|}{\centering\textbf{Total Non-Standard Error}} &
{\centering\arraybackslash\textbf{4}} \\ \hline

\end{longtable}
